...
// CONSTRUCTORS
Book::Book(const std::string& name, const int& identifier, const std::string& author) :
    name_(name), identifier_(identifier), author_(author), book_state_(available) {}
 ...
 */
BookState Book::GetBookState() const {
  return book_state_;
}
 ...
 */
void Book::SetBookState(const BookState& value) {
  book_state_ = value;
}
